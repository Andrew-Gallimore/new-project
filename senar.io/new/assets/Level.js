...
//moveing camera around
var camTime = new Date().getTime();
var camTimeOffsetMax = 900;
var camTimeOffsetMin = 550;
...
//outputting player positions
var oldPosTime = 0;
var startPosSent = false;
var oldPlayerX;
var oldPlayerY;
...
var operationValues = [1, "end"];
var operationCounter = 0;

//testing scaleing of the game, and how that effects performance
var TestingOffset = 0;
var startTime = new Date().getTime();
var TestValues1 = [0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 1];
var TestCount1 = [3000,5000,7000,9000,11000,13000,15000,17000,19000,21000, "end"]

var fps = [];
var pack = {name: Object.values(JSON.parse(localStorage.getItem('lastUser')))[0], username: Object.values(JSON.parse(localStorage.getItem('lastUser')))[1], is_mobile: this.mobile, results: []};
var counterTest = 0;
var started = false;
...
class Level extends Phaser.Scene {
	...
	update(time, delta) {
		...
		playerMovement(this.player, delta);
		...
		// if((new Date().getTime()) - camTime > 2000 && (new Date().getTime()) - camTime < 2040) {
		// 	//console.log("new!")
		// 	fittingGame(0.6);
		// 	this.game.scale.resize(gameWidth, gameHeight);
		// }
		testing(this.game.loop.actualFps);
		//console.log(this.game.scene.scenes[1])
		var keyObj = this.game.scene.scenes[1].input.keyboard.addKey('X');  // Get key object
		if(keyObj.isDown) {
			started = true;
			TestingOffset = -(new Date().getTime() - startTime);
			console.log("starting")
		}
		// console.log(keyObj.isDown);
		//console.log((new Date().getTime()) - camTime)
	}
	/* END-USER-CODE */
}
...
function playerMovement(player, delta) {
	const speedX = 400;
	var endSpeedX = speedX;
	const upX = 1450;
	const downX = 0.01;

	const speedY = 340;
	var endSpeedY = speedY;
	const upY = 1700;
	const downY = 0.007;
	...
	//for if the device is in mobile
	...
	this.downM = false;
	//horisontal movement
	if(Math.abs(this.joy.GetX()) > 20 && (Math.abs(this.joy.GetX()) < 70)) {
		if(this.joy.GetX() < 0) {
			this.leftM = true;
			endSpeedX = 0.5 * speedX;
		}
		if(this.joy.GetX() > 0) {
			this.rightM = true;
			endSpeedX = 0.5 * speedX;
	...
	//virtical movement
	if(Math.abs(this.joy.GetY()) > 20 && (Math.abs(this.joy.GetY()) < 70)) {
		if(this.joy.GetY() < 0) {
			this.downM = true;
			endSpeedY = 0.5 * speedY;
		}
		if(this.joy.GetY() > 0) {
			this.upM = true;
			endSpeedY = 0.5 * speedY;
	...
	body.setMaxVelocityX(endSpeedX);
	...
	if (this.leftM == false && this.rightM == false) {
		body.setAccelerationX(0);
		body.setDragX(downX / delta);
	}

	body.setMaxVelocityY(endSpeedY);
	...
	if (this.upM == false && this.downM == false) {
		body.setAccelerationY(0);
		body.setDragY(downY / delta);
	}
	//console.log(this.joy.GetX())
}

function outputPos(playerx,playery) {
	if(this.socketID !== undefined) {
		var curTime = new Date().getTime();
		var time = (curTime - camTime) / (1000/12);

		//limiting the outputting of positons to 12 fps
		if((time - oldPosTime) > 1) {
			//in the first frame output player positon
			if(startPosSent === false) {
				...
				//updateing old player positions
				oldPlayerX = playerx;
				oldPlayerY = playery;
				startPosSent = true;
			}

			if(Math.abs(oldPlayerX - playerx) > 0 || Math.abs(oldPlayerY - playery) > 0) {
				//putting out player position
				var pack = [];
				pack.push({
					ID:"" + this.socketID,
...
function mapCam(width, height) {
	...
	if(width < maxWidth && width > minWidth) {
		camTimeOffset = map(width, minWidth, maxWidth, camTimeOffsetMin, camTimeOffsetMax);
	...
	if(height < maxHeight && height > minHeight && height < width) {
		camTimeOffset = map(height, minHeight, maxHeight, camTimeOffsetMin, camTimeOffsetMax);
		//maping camera waiting time from between ...OffsetMin & ...OffsetMax based off distance between minwidth/height
...
function testing(curentfps) {
	if(testingComplete == false && started == true) {
		// console.log(new Date().getTime() + TestingOffset - startTime);
		// if(Object.values(JSON.parse(localStorage.getItem('testing')))[0] === "")
		//if we hit the end of the thing we are testing
		//console.log(TestCount1[counterTest + 1])
		if(TestCount1[counterTest + 1] === "end") {
			//and if we are at the end of the operations, end, else keep going
			if(operationValues[operationCounter + 1] === "end") {
				testingComplete = true;
				console.log("Testing: Completed!");
				console.log(pack)
				this.socket.emit('testing',pack);
				alert("Completed the Test!");
			}else {
				operationCounter++;
				counterTest = 0;
				console.log("Testing: Next Operation...");
			}
		}else {
			if(operationCounter === 0) {
				if((new Date().getTime()) + TestingOffset - startTime > TestCount1[counterTest] && (new Date().getTime()) + TestingOffset - startTime < TestCount1[counterTest + 1]) {
					//the actual test
					fittingGame(TestValues1[counterTest]);
					this.game.scale.resize(gameWidth, gameHeight);
					//working with loging results
					var value = TestValues1[counterTest];
					var camData_number = camData.length;
					var number_of_people = this.pnum;
					pack.results.push({test: {value, fps, camData_number, number_of_people}})
					fps = [];

					//working with the counter
					console.log("Testing: > Operation: " + operationCounter + " > Step: " + counterTest);
					counterTest++;
				}
			}
			if(operationCounter === 1) {
				if((new Date().getTime()) - startTime > TestCount1[counterTest] && (new Date().getTime()) + TestingOffset - startTime < TestCount1[counterTest + 1]) {
					console.log("Testing: > Operation: " + operationCounter + " > Step: " + counterTest);
					counterTest++;
					//the actual test
				}
		...
		// name: Object.values(lastUser)[0],
		// username: Object.values(lastUser)[1],
	}
}